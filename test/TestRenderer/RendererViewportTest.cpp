...
TEST(RendererViewportTest, ReservesLeftSidebarInLogicalAndFramebufferSpace) {
    const renderer::SceneViewport viewport = renderer::Renderer::calculate_scene_viewport(
        {1000, 500}, {2000, 1000}, scene_minus_left_sidebar(1000.0, 500.0, 328.0));
...
TEST(RendererViewportTest, KeepsValidViewportWhenSidebarConsumesAlmostAllSpace) {
    ...
    const renderer::SceneViewport viewport = renderer::Renderer::calculate_scene_viewport(
        {200, 100}, {400, 200}, renderer::LogicalViewportRect{500.0, 0.0, 200.0, 100.0});
...
TEST(RendererViewportTest, FullWindowRectFillsFramebuffer) {
    const renderer::SceneViewport viewport = renderer::Renderer::calculate_scene_viewport(
        {1000, 500}, {2000, 1000}, renderer::LogicalViewportRect{0.0, 0.0, 1000.0, 500.0});
...
TEST(RendererViewportTest, MapsTopOffsetRectToFramebufferPixels) {
    ...
    const renderer::SceneViewport viewport = renderer::Renderer::calculate_scene_viewport(
        {1000, 500}, {2000, 1000}, renderer::LogicalViewportRect{0.0, 100.0, 1000.0, 400.0});
