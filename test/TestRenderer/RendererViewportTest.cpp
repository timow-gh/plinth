...
TEST(RendererViewportTest, ReservesLeftSidebarInLogicalAndFramebufferSpace) {
    const renderer::SceneViewport viewport =
        plinth::plinth::calculate_scene_viewport({1000, 500}, {2000, 1000}, 328.0);
...
TEST(RendererViewportTest, TranslatesWindowCoordinatesToSceneFramebufferCoordinates) {
    ...
    const auto leftEdge = plinth::plinth::to_scene_framebuffer_coordinates(viewport, 328.0, 250.0);
    ASSERT_TRUE(leftEdge.has_value());
    EXPECT_DOUBLE_EQ(leftEdge->first, 0.0);
    EXPECT_DOUBLE_EQ(leftEdge->second, 500.0);

    const auto scenePoint = plinth::plinth::to_scene_framebuffer_coordinates(viewport, 400.0, 100.0);
...
TEST(RendererViewportTest, RejectsSidebarAndOutsideWindowCoordinates) {
    ...
    EXPECT_FALSE(plinth::plinth::to_scene_framebuffer_coordinates(viewport, 327.0, 250.0).has_value());
    EXPECT_FALSE(plinth::plinth::to_scene_framebuffer_coordinates(viewport, 1000.0, 250.0).has_value());
    EXPECT_FALSE(plinth::plinth::to_scene_framebuffer_coordinates(viewport, 400.0, 500.0).has_value());
}

TEST(RendererViewportTest, KeepsValidViewportWhenSidebarConsumesAlmostAllSpace) {
    const renderer::SceneViewport viewport =
        plinth::plinth::calculate_scene_viewport({200, 100}, {400, 200}, 500.0);
