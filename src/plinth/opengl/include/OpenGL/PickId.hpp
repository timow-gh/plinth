...
[[nodiscard]]
inline constexpr std::array<float, 3> encode_pick_index(std::uint32_t index) noexcept {
...
// "no hit" clear color.
[[nodiscard]]
inline constexpr std::uint32_t decode_pick_index(std::uint8_t r, std::uint8_t g, std::uint8_t b) noexcept {
