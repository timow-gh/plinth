...
std::optional<Texture2D> Texture2D::create(const renderer::TextureData& data, int maxTextureSize, int maxAnisotropy) {
    if (maxTextureSize < 1 || data.width == 0 || data.height == 0 ||
        data.width > static_cast<std::uint32_t>(maxTextureSize) ||
...
bool Texture2D::is_valid() const noexcept {
    return m_id != 0;
}
GLuint Texture2D::get_id() const noexcept {
    return m_id;
}
int Texture2D::get_width() const noexcept {
    return m_width;
}
int Texture2D::get_height() const noexcept {
    return m_height;
}
